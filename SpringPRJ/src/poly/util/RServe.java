...
public class RServe {
	...
	public List<CONSUMER_RcmmndFtDTO> usrEcldDist(List<CONSUMER_FtReviewDTO> mainUserRvDTO, List<CONSUMER_FtReviewDTO> compUsersRvDTO) throws REXPMismatchException{
        ...
        try {
            ...
            //유클리드 거리 측정
            connection.eval("euclidDist <- as.matrix((dist(t(resultMat))))"); 
            
            //첫번째 행만 가져오기 
            connection.eval("euclidDistResult <- euclidDist[2:ncol(euclidDist)]"); 
    		...
    		for(int i = 1; i <= userSeqsLen; i++) {
            	...
            	rftDTO.setRcmmndRating(connection.eval("euclidDistResult[" + i + "]").asString()); //결과 값
            	rftDTO.setUserSeq(connection.eval("userSeqList[" + i + "]").asString()); //결과 값
            	System.out.println(rftDTO.getRcmmndRating() + " ===== " + rftDTO.getUserSeq());
            	
            	if(rftDTO.getRcmmndRating().equals("0.0")) continue;
	...
	public List<CONSUMER_RcmmndFtDTO> usrEcldDist(List<CONSUMER_FtReviewDTO> mainUserRvDTO, List<CONSUMER_FtReviewDTO> compUsersRvDTO) throws REXPMismatchException{
		...
		try {
			...
			for(int i = 1; i <= userSeqsLen; i++) {
				...
				rftDTO.setUserSeq(connection.eval("userSeqList[" + i + "]").asString()); //결과 값
//            	System.out.println(rftDTO.getRcmmndRating() + " ===== " + rftDTO.getUserSeq());
				if(rftDTO.getRcmmndRating().equals("1.0")) continue;
				rftDTOArr.add(rftDTO);
			}
