	...
	<head>
		...
		<script>
			function pageSizeEdit(pageSize){
				location.href="<%=request.getContextPath()%>/admin/user/user_activity_search_list.do?option=<%=option%>&value=<%=value%>&pageSize="+pageSize+"&pageNum=<%=pageNum%>";
			}
			...
			function checkUser(mode){
				...
				if(mode==1 && check_Cnt != 0){
					location.href="<%=request.getContextPath()%>/admin/user/user_activity_active.do?ArrUser_Seq="+check_Arr;
				}else if(mode==2 && check_Cnt != 0){
					location.href="<%=request.getContextPath()%>/admin/user/user_activity_drop.do?ArrUser_Seq="+check_Arr;
				}else if(mode==3 && check_Cnt != 0){
					location.href="<%=request.getContextPath()%>/admin/user/user_activity_auth.do?ArrUser_Seq="+check_Arr+"&ArrUser_Auth="+auth_Arr;
				}
			}
		</script>
	</head>
	...
	<body>
		<div style="position:relative; float:left; width:68%;">
			<p>검색된 회원 수 : <%=uDTOArr.size()%></p>
                   선택 회원&nbsp; 
                   <input type="button" value="활동정지 해제" class="btn btn-default" onClick="javascript:checkUser(1)"> 
                   <input type="button" value="강제탈퇴 해제" class="btn btn-default" onClick="javascript:checkUser(2)">
		...
		<table id="htmlgrid" class="testgrid" style="margin-top:10px;">
			...
			<%
				if(uDTOArr.size()==0 || uDTOArr==null){%>
					<tr height="80px"><td align="center" colspan="8">정지/탈퇴 회원이 없습니다.</td></tr>	
				<%}else{
		...
		<div  id="edition"  align="center"  style="clear:both;">
        	<%if (uDTOArr.size()>0){
				...
				if (startPage>pageBlock){%>
					<input type="button" value="<" class="btn btn-default" onClick="location.href='<%=request.getContextPath()%>/admin/user/user_activity_search_list.do?option=<%=option%>&value=<%=value%>&pageNum=<%=startPage-pageBlock%>&pageSize=<%=pageSize%>''">
	<%		}
				for(int i=startPage; i<=endPage; ++i){%>
				<%if(Integer.parseInt(pageNum)==i){ %>
					<input type="button" value="<%=i %>" class="btn btn-primary"  onClick="location.href='<%=request.getContextPath()%>/admin/user/user_activity_search_list.do?option=<%=option%>&value=<%=value%>&pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%}else{ %>
					<input type="button" value="<%=i %>" class="btn btn-default"  onClick="location.href='<%=request.getContextPath()%>/admin/user/user_activity_search_list.do?option=<%=option%>&value=<%=value%>&pageNum=<%=i%>&pageSize=<%=pageSize%>'">		
				<%} %>
	<%		}
